	...
	function DataObject:Update()
		...
		for name, module in self.core:IterateModules() do
			if module:IsEnabled() and type(module.FeedDataObject) == 'function' then
				...
				if modWeight then
					SimpleSelfRebuff:Debug('%s: weight=%s, text=%q icon=%q', module:GetName(), modWeight, modText, modIcon)
		...
		SimpleSelfRebuff:Debug('Finally: text=%q (%s), icon=%q (%s)', text, textWeight, icon, iconWeight)
		object.text = text
		object.icon = icon
	end
